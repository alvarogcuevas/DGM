...
new_step_original<-function(Z,s,A,E_n,i,L,h,varepsilon,TT){
  ...
  if(A_temp >= E_n){
    ...
    Z<-max(0,min(Z_tau_minus+varepsilon*gamma_fn(y = random_mark()),1))
    s<-tau_n
    A<-E_n
...
chain<-function(Z,s,A,E_n,i,L,T_,t0,epsilon,Nsteps,c_,new,stop.at.tau){
  #h<-(T_-t0)/Nsteps
