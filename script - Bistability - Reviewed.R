...
source('~/Concordia/Research/Git/DGM/functions - Bistability - Reviewed.R')
...
#This is for doing a long graph
initial<-data.frame(Z=x0,s=low_t,A=0,E_n=rexp(n = 1,rate = 1),i = 0,L = 1)
time<-low_t
big_chain_Z<-big_chain_s<-indicators<-c()
small_chain<-chain(Z = initial$Z,s = initial$s,A = initial$A,E_n = initial$E_n,i = initial$i,L = initial$L,T_ = T_vec,t0 = low_t,epsilon = varepsilon_vec,Nsteps = N_steps,c_ = 0,new = F,stop.at.tau=F)
